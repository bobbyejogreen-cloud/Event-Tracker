...
async function deleteCalendarEvent(auth, calendarId, calendarEventId) {
  const calendar = google.calendar({ version: 'v3', auth });
  try {
    ...
    // Ignore 404/410 — event was already deleted or gone
...
async function purgeAllEvents(auth, calendarId) {
  const calendar = google.calendar({ version: 'v3', auth });
  let deleted = 0;
  let pageToken;
  ...
  do {
    ...
    for (const ev of events) {
      // Delete events that were created by event-watch
      const isManagedByUs = ev.description && ev.description.includes('Managed by event-watch');
      if (isManagedByUs) {
        try {
          ...
          if (err.code !== 404 && err.status !== 404) {
            console.log(`  Failed to delete "${ev.summary}": ${err.message}`);
          }
        }
      }
    }
...
module.exports = { upsertCalendarEvent, createCalendarEvent, updateCalendarEvent, deleteCalendarEvent, purgeAllEvents, ensureCalendar };
